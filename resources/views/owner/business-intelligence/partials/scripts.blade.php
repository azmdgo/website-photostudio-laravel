...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize cancellation reason chart - direct approach
    console.log('Initializing cancellation reason chart...');
    
    // Wait for all images and resources to load
    window.addEventListener('load', () => {
        setTimeout(() => {
            console.log('Window loaded, initializing cancellation chart...');
...
function updateCancellationReasonChart() {
    console.log('updateCancellationReasonChart called');
    const filterSelect = document.getElementById('cancellationReasonChartFilter');
    console.log('Filter select found:', filterSelect);
    
    // Use default value if filter element is not found
    const months = filterSelect ? filterSelect.value : 12;
    console.log('Months selected:', months);

    // Show loading state
    const chartCanvas = document.getElementById('cancellationReasonChart');
    if (!chartCanvas) {
    ...
    fetch(`{{ route('owner.business-intelligence.cancellation-reason-data') }}?months=${months}`)
        .then(response => {
            console.log('Cancellation reason fetch response status:', response.status);
...
// Debug: Log data check details
    console.log('Entering data check stage...');
    console.log('cancellationReasonData.reasons:', cancellationReasonData.reasons);
    console.log('cancellationReasonData.has_data:', cancellationReasonData.has_data);

    // Check if there's data to display
    if (!cancellationReasonData.reasons || cancellationReasonData.reasons.length === 0 || !cancellationReasonData.has_data) {
        // Debug: Log empty state creation
        console.warn('No data available, showing empty chart state.');
...
function updateServiceChart() {
    ...
    // Fetch new data via AJAX
    fetch(`{{ route('owner.business-intelligence.service-data') }}?months=${months}`)
...
function updateCategoryChart() {
    ...
    // Fetch new data via AJAX
    fetch(`{{ route('owner.business-intelligence.service-category-data') }}?months=${months}`)
        .then(response => response.json())
        .then(data => {
...
function updateStatusChart() {
    ...
    // Fetch new data via AJAX
    fetch(`{{ route('owner.business-intelligence.status-data') }}?months=${months}`)
        .then(response => response.json())
        .then(data => {
...
function updateRevenueChart() {
    ...
    const months = parseInt(filterSelect ? filterSelect.value : 12);
    ...
    // Fetch new data via AJAX
    fetch(`{{ route('owner.business-intelligence.revenue-data') }}?months=${months}`)
        .then(response => response.json())
        .then(data => {
...
function updateCustomerGrowthChart() {
    ...
    // Fetch new data via AJAX
    fetch(`{{ route('owner.business-intelligence.customer-growth-data') }}?months=${months}`)
        .then(response => response.json())
        .then(data => {
...
function exportToFormat(format) {
    ...
    let exportUrl;
    if (format === 'pdf') {
        exportUrl = '{{ route("owner.business-intelligence.export-pdf") }}';
    } else if (format === 'excel') {
        exportUrl = '{{ route("owner.business-intelligence.export-excel") }}';
    } else if (format === 'csv') {
        exportUrl = '{{ route("owner.business-intelligence.export-excel") }}' + '?export_format=csv';
    }
...
function generateAIInsights() {
    ...
    fetch('{{ route("owner.business-intelligence.ai-insights") }}', {
        method: 'GET',
        headers: {
...
function updateAIPredictionCards(data) {
    ...
    if (data && data.success && data.predictions) {
        ...
        if (predictions.revenue !== undefined) {
        ...
        if (predictions.bookings !== undefined) {
            const bookingsElement = document.getElementById('predictedBookings');
            if (bookingsElement) {
                const roundedBookings = Math.round(predictions.bookings);
        ...
        if (predictions.customers !== undefined) {
            const customersElement = document.getElementById('predictedCustomers');
            if (customersElement) {
                const roundedCustomers = Math.round(predictions.customers);
...
// Helper function to format table values based on type
function formatTableValue(value, type) {
    if (value === null || value === undefined || value === '') {
        return 'N/A';
    }
    
    // If it's already a formatted string from PHP, just add currency symbol for revenue
    if (typeof value === 'string' && value !== 'N/A') {
        if (type && type.toLowerCase().includes('pendapatan')) {
            // Add Rupiah symbol to pre-formatted number
            return 'Rp ' + value;
        } else {
    ...
    if (typeof value === 'number') {
        ...
        } else if (type && (type.toLowerCase().includes('pelanggan') || type.toLowerCase().includes('pemesanan'))) {
            // Format sebagai angka biasa dengan separator
            return new Intl.NumberFormat('id-ID').format(value);
        } else {
            // Default format dengan separator
            return new Intl.NumberFormat('id-ID').format(value);
        }
    }
...
// Helper function to format weighted values
function formatTableWeightedValue(value, type) {
    if (value === null || value === undefined || value === '') {
        return 'N/A';
    }
        ...
        if (details && details.breakdown && Array.isArray(details.breakdown)) {
            details.breakdown.forEach(item => {
                ...
                row.innerHTML = `
                    ...
                    <td>${formatTableValue(item.value, details.type)}</td>
                    <td class="text-center">${item.weight || 'N/A'}</td>
                    <td>${formatTableWeightedValue(item.weighted_value, details.type)}</td>
                    <td class="text-center">
            ...
            const formattedPrediction = formatTableValue(prediction, details.type);
